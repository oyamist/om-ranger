...
enum Status {
    ...
    STATUS_NO_MOCK = -431,		    // Mock TinyThreads does not handle this command

    // ITinyThreads
    STATUS_OPEN = -500,				// Expected prior call to ITinyThreads::open()
    STATUS_IFIRESTEP = -501,		// Expected non-null pointer to ITinyThreads instance
    STATUS_USB_OPEN = -502,			// ArduinoUSB::open() failed
    STATUS_USB_CLOSE = -503,		// ArduinoUSB::close() failed
    STATUS_TIMEOUT = -504,			// ITinyThreads::execute() timed out
    STATUS_USB_CONFIGURE = -505,	// Could not configure serial port
    STATUS_REQUEST_LF = -506,		// ITinyThreads::execute() request must end with LF
